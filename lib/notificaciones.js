...
export async function registrarToken(usuarioBarId) {
  try {
    ...
    const { data: existente, error: errorBuscar } = await supabase
      .from('push_tokens').select('id, token').eq('usuario_bar_id', usuarioBarId).maybeSingle()

    if (errorBuscar) {
      Alert.alert('No se pudo revisar tu token de notificaciones', errorBuscar.message)
      return
    }

    if (existente) {
      if (existente.token === token) return
      const { error } = await supabase.from('push_tokens').update({ token }).eq('id', existente.id)
      if (error) Alert.alert('No se pudo actualizar tu token de notificaciones', error.message)
    } else {
      const { error } = await supabase.from('push_tokens').insert({ usuario_bar_id: usuarioBarId, token })
